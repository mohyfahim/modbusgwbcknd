...
void mbbk_register_routes(struct _u_instance* instance, void* user_data){
  ...
  ulfius_add_endpoint_by_val(instance, "POST", API_PREFIX "login", NULL, 0,
                             &mbbk_route_user_login, user_data);

  ulfius_add_endpoint_by_val(instance, "GET", API_PREFIX "user", NULL, 0,
                             &mbbk_route_user_get_all, user_data);
}